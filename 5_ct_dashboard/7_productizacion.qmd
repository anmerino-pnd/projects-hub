...
### 4.4 Pipeline → dashboard: el contrato de los 8 parquets
...
El contrato entre pipeline y dashboard son los **ocho parquets** generados en `datos/processed/`:
...
└── temp_diario.parquet              # ~320 filas (90 días × 5 segmentos) — vista "Último mes"
```
...
### 4.5 Una vista por dentro: el historial del cliente

Para aterrizar cómo se ve una vista, reproducimos (con datos simulados) el **drill-down de cliente**: su historial **completo** de pedidos. Antes la gráfica se truncaba a los últimos 50 pedidos; ahora muestra toda la vida del cliente, y el detalle de cada pedido —monto, **productos únicos** y **unidades**— vive en el hover en lugar de una tabla aparte.

```{python}
#| label: fig-cliente-historial
#| fig-cap: "Historial de pedidos de un cliente — réplica de la vista de drill-down (datos simulados). El hover muestra monto, productos únicos y unidades."
...
# ~40 pedidos de un cliente a lo largo de ~3 años (historial completo, sin truncar a 50)
pool = pd.date_range("2023-05-01", "2026-05-20", freq="D").to_numpy()
fechas = np.sort(rng.choice(pool, size=40, replace=False))
pago = np.round(rng.lognormal(mean=10.8, sigma=0.9, size=40), 2)   # MXN
prod_unicos = rng.integers(1, 6, size=40)
unidades = prod_unicos * rng.integers(5, 40, size=40)

fig = go.Figure(go.Scatter(
    x=fechas, y=pago, mode="lines+markers",
    line=dict(color="#0B3C5D", width=1), marker=dict(size=7, color="#0B3C5D"),
    customdata=np.stack([prod_unicos, unidades], axis=1),
...
fig.update_layout(
    ...
    xaxis=dict(title="", type="date", dtick="M6", tickformat="%b %y", tickangle=0),
    yaxis=dict(title="Pago total (MXN)", tickformat=",.0f"),
    margin=dict(t=20, r=20, b=50, l=70),
)
fig.show()
```
:::
